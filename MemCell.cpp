...
double MemCell::CalculateReadPower() { /* TO-DO only considers NVM cells */
	if (readEnergy == 0) {
		if (readVoltage == 0) {
			readVoltage = MIN(tech->vdd, readCurrent * resistanceOn + voltageDropAccessDevice);
		}
		if (readCurrent == 0) {
			readCurrent = (readVoltage - voltageDropAccessDevice) / resistanceOn;
		}
		return readVoltage * readCurrent;
	} else
		return -1; /* should not call the function if read energy exists */
}
